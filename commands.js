...
const HELP_EN = [
  ...
  '',
  '**⏱ Boss Timer**',
  '`kill <bossname>` / `<bossname> cut` → Record boss kill using current JST.',
  '> Examples: `kill Venatus` \u2022 `Venatus cut`',
...
const HELP_KO = [
  ...
  '',
  '**⏱ 보스 타이머**',
  '`처치 <보스명>` / `<보스명> 컷` → 현재 JST 기준으로 보스 처치를 기록합니다.',
  '> 예시: `처치 베나투스` \u2022 `베나투스 컷`',
...
const HELP_JA = [
  ...
  '',
  '**⏱ ボスタイマー**',
  '`討伐 <ボス名>` / `<ボス名> カット` → 現在のJSTでボス討伐を記録します。',
  '> 例: `討伐 ベナトゥス` \u2022 `ベナトゥス カット`',
