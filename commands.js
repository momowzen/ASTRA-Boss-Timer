...
const HELP_EN = [
  ...
  '',
  '**Guild Management**',
  '`/addguild guild_names:<name1> <name2>...` → Register guilds for rotation.',
  '`/addguild remove_guild:<name>` → Remove a guild.',
  '`/addguild action:clear` → Clear all guilds.',
  '`/addguild action:list` → List registered guilds.',
  '`/assignboss guild:<guild>` → Assign bosses to a guild (opens modal).',
  '',
  '**Notifications**',
...
const HELP_KO = [
  ...
  '',
  '**길드 관리**',
  '`/길드추가 guild_names:<이름1> <이름2>...` → 로테이션용 길드 등록.',
  '`/길드추가 remove_guild:<이름>` → 길드 제거.',
  '`/길드추가 action:clear` → 모든 길드 삭제.',
  '`/길드추가 action:list` → 등록된 길드 목록 표시.',
  '`/보스배정 guild:<길드>` → 길드에 보스 배정 (모달 열림).',
  '',
  '**알림**',
  '`/설정` → 알림 채널을 설정합니다.',
...
const HELP_JA = [
  ...
  '',
  '**ギルド管理**',
  '`/ギルド追加 guild_names:<名前1> <名前2>...` → ローテーション用ギルド登録。',
  '`/ギルド追加 remove_guild:<名前>` → ギルド削除。',
  '`/ギルド追加 action:clear` → すべてのギルドをクリア。',
  '`/ギルド追加 action:list` → 登録済みギルド一覧を表示。',
  '`/ボス割当 guild:<ギルド>` → ギルドにボスを割り当て（モーダル表示）。',
  '',
  '**通知**',
  '`/せってい` → 通知チャンネルを設定します。',
