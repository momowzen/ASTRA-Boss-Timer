...
function migrateConfig(data) {
  if (data.rotation && typeof data.rotation === 'object' && !data.rotation.type) {
    ...
    const order = Object.keys(oldGuildNames).map(String);

    const bossGuild = {};
    for (const [bossId, guildNum] of Object.entries(oldRotation)) {
      const guildName = oldGuildNames[String(guildNum)] || String(guildNum);
      bossGuild[bossId] = guildName;
    }

    const newGuildNames = {};
    for (const [num, name] of Object.entries(oldGuildNames)) {
      newGuildNames[name] = name;
    }
    ...
    data.guildNames = newGuildNames;
  }

  if (data.rotation) {
    if (!data.rotation.type) data.rotation.type = null;
    if (!data.rotation.order) data.rotation.order = [];
    if (!data.rotation.activeIdx && data.rotation.activeIdx !== 0) data.rotation.activeIdx = 0;
    if (!data.rotation.bossGuild) data.rotation.bossGuild = {};
    if (!data.rotation.flipDay && data.rotation.flipDay !== 0) data.rotation.flipDay = null;
    if (!data.rotation.flipHour && data.rotation.flipHour !== 0) data.rotation.flipHour = null;
    if (!data.rotation.flipMinute && data.rotation.flipMinute !== 0) data.rotation.flipMinute = null;
...
client.once('clientReady', async () => {
  ...
  // Weekly rotation flip check (every 15 minutes)
  setInterval(async () => {
    try {
      const rot = config.rotation || {};
      if (rot.type !== 'weekly' || !rot.order || rot.order.length < 2) return;
      ...
      if (daysSinceFlipDay === 0) {
        ...
        if (diff >= 0 && diff < 900) {
          ...
          if (lastRotatedAt < flipTimestamp) {
            ...
            rot.activeIdx = ((rot.activeIdx || 0) + 1) % rot.order.length;
            const newGuild = rot.order[rot.activeIdx];
      ...
      console.error('[ROTATION] Weekly flip error:', e);
    }
  }, 900000);
